...
export class MoonshotDialogueProvider implements DialogueProvider {
  ...
  constructor(options: MoonshotProviderOptions) {
    ...
    this.endpoint = z
      .string()
      .url()
      .parse(options.endpoint ?? MOONSHOT_CHAT_COMPLETIONS_URL);
  }
  ...
  async complete(input: DialogueCompletionInput): Promise<DialogueCompletion> {
    ...
    try {
      ...
      const parsedResponse = MoonshotResponseSchema.safeParse(
        await response.json(),
      );
...
export function createMoonshotDialogueProvider() {
  if (!env.moonshotApiKey) {
    throw new DialogueProviderError("provider_unavailable", false);
  }

  return new MoonshotDialogueProvider({
