...
服务器只忽略流中的部分预览，只接受一个 `image_generation.completed` 事件和随后
的 `[DONE]`。只有最终事件中正好包含一张有效的 `b64_json` 图片，才算成功。
服务器解码图片后，会检查图片文件本身的开头标记，而不是完全相信接口写的图片类型。

允许 PNG、JPEG 和 WebP，解码后的图片最大不能超过 20 MiB。

OpenRouter 图片生成最多等待 160 秒。整个 Next.js 接口最多运行 180 秒，给图片检查、上传和数据库保存留下时间。流式返回只用于保持长生成请求可观察和避免上游缓冲超时，浏览器仍然不会展示半成品。
